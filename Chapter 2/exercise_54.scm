...
;equality of symbols by saying that a and b are `equal?` if they are both symbols 
;and the symbols are `eq?`, or if they are both lists such that `(car a)` is `equal?` to `(car b)` and `(cdr a)` 
