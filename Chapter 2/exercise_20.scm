...
;as usual, but the final parameter's value will be a list of any remaining arguments. 
...
(define (filter collection predicate)
        (cond ((null? collection) '())
              ((predicate (car collection)) (cons (car collection) 
                                                  (filter (cdr collection) 
                                                          predicate)))
              (else (filter (cdr collection) predicate))))


(define (same-parity . integers)
        (let ((parity (even? (car integers))))
             (if parity 
