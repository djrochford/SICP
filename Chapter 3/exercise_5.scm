...
;(x - 5)^2 + (y - 7)^2 < 32. To estimate the area of the region described by such a predicate,
;begin by choosing a rectangle that contains the region. For example, a rectangle with
;diagonally opposite corners at (2, 4) and (8, 10) contains the circle above. The desired
;integral is the area of that portion of the rectangle that lies in the region. We can
;estimate the integral by picking, at random, points (x,y) that lie in the rectangle, and
;testing P(x, y) for each point to determine whether the point lies in the region. If we try
;this with many points, then the fraction of points that fall in the region should give an
...
;a predicate `P`, upper and lower bounds `x1`, `x2`, `y1`, and `y2` for the rectangle, and
;the number of trials to perform in order to produce the estimate. Your procedure should use
;the same `monte-carlo` procedure that was used above to estimate pi. Use your
;`estimate-integral` to produce an estimate of pi by measuring the area of a unit circle.
